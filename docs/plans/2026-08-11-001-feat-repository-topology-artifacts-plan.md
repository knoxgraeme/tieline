...
- **Long publication transaction:** Rich analysis happens before SQL, but the full-envelope fact import remains transactional. The pinned 5,000-file fixture measured 109.3 seconds for normalized same-host persistence after bounded COPY batching, so keep 150 seconds as the cold topology-import sub-budget and set a measured end-to-end ceiling below the deployment timeout for cold and reuse paths before rollout. This is a merge-only publication budget; reconsider durable staging only in a separate plan if the combined ceiling cannot be met.
