...
- R25. Existing databases must advance through ordered additive migrations after immutable `0001_baseline.sql`; the baseline checksum must never change. Administrative migration supports an explicit target version: apply through `0002_repository_accepted_state.sql` for expansion, then through `0003_retire_legacy_topology_checkpoint.sql` only after exact-main bootstrap, cutover, and rollback-floor advancement. Verification reports pending, ahead-of-target, or drifted migrations, and a sync-only publisher returns `schema_outdated` instead of running owner DDL.
...
- `migrations/0001_baseline.sql` owns the clean baseline, least-privilege grants, current independent checkpoints, and promotion functions.
...
- **Postgres lifecycle:** Rich generations and DB-native records remain separate. Before cutover, legacy checkpoints remain active only for the pre-cutover application while legacy publication automation is disabled. After cutover and `0003`, supported reads, writes, and retention use only the combined checkpoint.
...
- **Dependencies:** U10.
- **Files:** Create `migrations/0002_repository_accepted_state.sql` and migration upgrade fixtures; modify `src/commands/migrate.ts`, `scripts/test-baseline.ts`, `scripts/integration-baseline.ts`, `package.json`, and schema/grant assertions that currently require exactly one migration.
- **Approach:**
...
- **Test Scenarios:**
  - A clean database applies `0001` then `0002`; an exact populated `0001` database applies only `0002`.
  ...
  - Pre/post digests preserve Stories, ACs, planning revisions, evidence, observations, backlog, attribution state, handoff conflicts, topology history, and every existing grant through expand-only `0002`.
...
- **Verification:** PostgreSQL 16 clean-install and populated-upgrade tests through target `0002` produce matching schema/privilege signatures. Rollback may use the old application only while legacy automation remains disabled; U7 owns the later retirement migration and combined-only rollback floor.
...
- **Approach:**
  ...
  2. Execute the rollout in this order: Phase 1 artifact gate on PR, merge queue, and `main`; administrative migration through expand-only `0002`; publisher-only combined-capable deployment while hosted selectors remain legacy and legacy publication automation is disabled; exact protected-main bootstrap; hosted-reader deployment that selects only the combined checkpoint; rollback-floor advancement; package and apply `0003` legacy revocation; then merge-only publication enablement.
...
- **Test Scenarios:**
  ...
  - The rollout exercises legacy, bootstrapped-not-cut-over, and combined-only states. Rollback before `0003` keeps legacy automation disabled; rollback after `0003` restores a combined-checkpoint-compatible application and never resumes independent writers.
  - `0003` revokes DML/EXECUTE authority for both legacy checkpoints and topology promotion, moves retention to combined checkpoints plus explicit pins, and leaves every unrelated grant unchanged.
...
- Phase 2 is complete when U8 and U5-U7 pass after Phase 1, one exact protected-main tuple is bootstrapped, `0003` has retired both legacy authorities, and merge-only publication is enabled.
...
- U8 preserves the immutable baseline and installed database state through the target-aware runner and expand-only `0002` migration.
...
- U7 completes physical legacy-authority revocation through gated `0003` and leaves the rollout, recovery guidance, manifest, topology, README, skill, resources, server instructions, and accepted ACs consistent.
