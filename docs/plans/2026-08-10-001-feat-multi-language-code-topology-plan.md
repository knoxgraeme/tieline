...
- KTD16. **Preserve the immutable baseline and role separation.** Add topology objects, indexes, and grants in `migrations/0002_code_topology.sql`; use the existing privileged sync writer for committed generations and read roles for traversal. Governs R18-R20.
...
- **Dependencies:** None.
- **Files:** Add `src/domain/code-topology-store.ts`, `src/adapters/postgres/code-topology-repository.ts`, `src/adapters/fakes/fake-code-topology-store.ts`, `migrations/0002_code_topology.sql`, and topology composition wiring; preserve `migrations/0001_baseline.sql` and modify `scripts/test-baseline.ts` and `scripts/integration-baseline.ts`.
- **Approach:**
...
- **Approach:**
  ...
  4. Enforce Phase 1 budgets in fresh installed-package child processes on pinned Ubuntu x64 Node 20 CI: parser artifacts at or below 7 MiB unpacked, packed tarball delta at or below 7 MiB, production-install parser footprint at or below 10 MiB, median runtime plus four-language load at or below 2 seconds, worst run at or below 4 seconds, and the 116-file Tieline corpus parse/query at or below a 1.5-second median and 2-second worst run after initialization. The corpus limits include secure snapshot reads and fact materialization and retain headroom over the measured 1.15-second shared-runner median.
  5. Enforce Phase 2 budgets on a deterministic 5,000-file / 50-MiB / 100,000-symbol / 250,000-edge fixture: one generation builds within 60 seconds, sequential base/current analysis within 120 seconds, peak RSS growth stays within 768 MiB, committed bulk persistence completes within 150 seconds on same-host Postgres, and default-limit sparse or dense traversal plus AC join has p95 at or below 500 ms and worst latency at or below 2 seconds. The persistence ceiling reflects the measured 109.3-second normalized import on pinned Ubuntu CI, including constraints, indexes, and WAL, with hosted-runner headroom; it is a merge-only publication budget rather than an interactive read budget.
